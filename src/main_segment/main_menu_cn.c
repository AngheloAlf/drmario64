...
const char STR_800C4C34_cn[] = "发生在马力欧医生和~n感冒狂人之间的精彩~n故事。~w2~h通过送给对手垃圾,~n逼迫他们退出游戏。~w2~h或消灭自己所有的病~n毒来赢得胜利。 ~w2~h再补充一点！~n游戏中可以按任意~n一个Ｃ键来观察药丸~n落下的位置，~w2~h再按一次可以取消这~n个效果。~w2~h~z";

const char STR_800C4D24_cn[] = "持续游戏直到在一局~n中失败。~w2~h消灭所有病毒才能过~n关。~w2~h游戏难度等级越高,~n需要消灭的病毒就越~n多。~w2~h再补充一点！~n游戏中可以按任意~n一个Ｃ键来观察药丸~n落下的位置，~w2~h再按一次可以取消这~n个效果。~w2~h~z";

const char STR_800C4DFC_cn[] = "这是你和电脑对抗的~n机会。~w2~h选定任意一个你喜欢~n的对手开始游戏。~w2~h通过送给对手垃圾, ~n逼迫他们退出游戏。~w2~h或消灭自己所有的~n病毒来赢得胜利。 ~w2~h再补充一点！~n游戏中可以按任意~n一个Ｃ键来观察药丸~n落下的位置，~w2~h再按一次可以取消这~n个效果。~w2~h~z";

const char STR_800C4F00_cn[] = "消灭所有的闪光病毒~n以取胜。~w2~h选定任意一个你喜欢~n的对手开始游戏。~w2~h通过送给对手垃圾, ~n逼迫他们退出游戏~w2~h或消灭自己所有的病~n毒来赢得胜利。 ~w2~h再补充一点！~n游戏中可以按任意~n一个Ｃ键来观察药丸~n落下的位置，~w2~h再按一次可以取消这~n个效果。~w2~h~z";

const char STR_800C5004_cn[] = "在这个模式中病毒会~n高速繁殖。~w2~h连消病毒可以降低~n它们的繁殖速度。~w2~h消灭病毒以保持瓶子~n清洁。~w2~h按L键可以加快病毒~n的繁殖。~w2~h再补充一点！~n游戏中可以按任意~n一个Ｃ键来观察药丸~n落下的位置，~w2~h再按一次可以取消这~n个效果。~w2~h~z";

const char STR_800C50F8_cn[] = "尽力在限定时间内争~n取最高分。~w2~h同时消灭很多病毒可~n争取最高分。~w2~h再补充一点！~n游戏中可以按任意~n一个Ｃ键来观察药丸~n落下的位置，~w2~h再按一次可以取消这~n个效果。~w2~h~z";

const char STR_800C51AC_cn[] = "请输入你的名字。~w2~h如果是第一次游戏，~n请选择：~n新游戏或玩家。~w2~h~z";

const char STR_800C51F8_cn[] = "请输入你的名字。~z";

const char STR_800C520C_cn[] = "和另外一个玩家比赛。~w2~h通过送给对手垃圾,~n逼迫他们退出游戏~w2~h或消灭自己所有的病~n毒来赢得胜利。 ~w2~h再补充一点！~n游戏中可以按任意~n一个Ｃ键来观察药丸~n落下的位置，~w2~h再按一次可以取消这~n个效果。~w2~h~z";

const char STR_800C52E0_cn[] = "消灭所有的闪光病毒~n以取胜。~w2~h完成连消能送给对手垃圾。~w2~h再补充一点！~n游戏中可以按任意~n一个Ｃ键来观察药丸~n落下的位置，~w2~h再按一次可以取消这~n个效果。~w2~h~z";

const char STR_800C5388_cn[] = "尽力在限定时间内取~n得最高分。~w2~h一次消灭多个病毒可~n得高分。~w2~h再补充一点！~n游戏中可以按任意~n一个Ｃ键来观察药丸~n落下的位置，~w2~h再按一次可以取消这~n个效果。~w2~h~z";

const char STR_800C5438_cn[] = "有多少玩家？~z";

const char STR_800C5448_cn[] = "四人自由比赛。~w2~h通过送给对手垃圾,~n逼迫他们退出游戏~w2~h或消灭自己所有的病~n毒来赢得胜利。 ~w2~h连消的颜色决定你的~n垃圾药丸送到哪里去。~w2~h再补充一点！~n游戏中可以按任意~n一个Ｃ键来观察药丸~n落下的位置，~w2~h再按一次可以取消这~n个效果。~w2~h~z";

const char STR_800C5544_cn[] = "二对二模式。~w2~h通过送给对手垃圾, ~n逼迫他们退出游戏，~w2~h或消灭自己所有的病~n毒来赢得胜利。 ~w2~h连消的颜色决定你的~n垃圾药丸送到哪里去。~w2~h送给同伴的垃圾药丸~n放置在你们~w2~h小型垃圾箱中，然后~n又加入到~w2~h你或你的同伴随后送~n给对手的垃圾中去。~w2~h再补充一点！~n游戏中可以按任意~n一个Ｃ键来观察药丸~n落下的位置，~w2~h再按一次可以取消这~n个效果。~w2~h~z";

const char STR_800C56B0_cn[] = "消灭所有的闪光病毒~n以取胜。~w2~h你可以将对手的屏幕~n塞满垃圾。~w2~h连消的颜色决定你的~n垃圾药丸送到哪里去。~w2~h再补充一点！~n游戏中可以按任意~n一个Ｃ键来观察药丸~n落下的位置，~w2~h再按一次可以取消这~n个效果。~w2~h~z";
...
const char STR_800C5984_cn[] = "如果你删除那个名字，~n那么就把相关的游戏~n数据也一同删掉了。~w2~h你真的想删除这个~n名字?~w2~h~z";
